FROM arm32v7/postgres:14-alpine
...
LABEL org.opencontainers.image.authors="PostGIS Project - https://postgis.net, Tobias Hargesheimer <docker@ison.ws>" \
	...
	org.opencontainers.image.description="Alpine with PostgreSQL 14 and PostGIS 3.1 on ARM arch" \
...
COPY --from=postgis/postgis:14-3.1-alpine /docker-entrypoint-initdb.d/10_postgis.sh /docker-entrypoint-initdb.d/10_postgis.sh
COPY --from=postgis/postgis:14-3.1-alpine /usr/local/bin/update-postgis.sh /usr/local/bin/update-postgis.sh
